...
def parse_clang_type(ct: Type, *, namespace: Optional[str] = None) -> SysType:
    def parse(ct: Type) -> SysType:
        ...
        match ct.kind:
            ...
            case TypeKind.POINTER:  # ty:ignore[unresolved-attribute]
                return PointerType(
                    is_const=is_const,
                    child=parse(ct.get_pointee()),
                    size=PointerSize.C,
                )
            case TypeKind.FUNCTIONPROTO:  # ty:ignore[unresolved-attribute]
                return FnType(
                    is_const=is_const,
                    arg_types=[parse(t) for t in ct.argument_types()],
                    ret_type=parse(ct.get_result()),
                )
            case TypeKind.INCOMPLETEARRAY:  # ty:ignore[unresolved-attribute]
                return PointerType(
                    is_const=is_const,
                    child=parse(ct.get_array_element_type()),
